...
export const githubStore = {
    ...
    async remove(model, id) {
        ...
        const updatedData = data.filter((item) => {
            return item.id !== id && item._id !== id && item.trackingId !== id;
        });
        await writeToGithub(`${model}.json`, updatedData, sha, `Delete ${model} ${id}`);
        return true;
    },

    async upsert(model, matchField, item) {
        const { data, sha } = await fetchFromGithub(`${model}.json`);
        const index = data.findIndex(i => i[matchField] === item[matchField] && i[matchField] !== undefined && i[matchField] !== null);
        ...
        await writeToGithub(`${model}.json`, updatedData, sha, `Upsert ${model}`);
        return item;
    }
};
